...
AuthorizeHandler.prototype.handle = function(request, response) {
  ...
  if ('false' === this.getAllowedParam(request)) {
...
AuthorizeHandler.prototype.getAllowedParam = function(request) {
  var allowed = request.body.allowed || request.query.allowed;

  if (!is.vschar(allowed)) {
    throw new InvalidRequestError('Invalid parameter: `allowed`');
  }
